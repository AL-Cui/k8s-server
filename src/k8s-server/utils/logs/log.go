...
// Tailing tails logs from redis adapter.
func (bl *BeeLogger) Tailing(ws *websocket.Conn, include, exclude string) {
	go reader(ws)
	var r *redisWriter
	for _, provider := range bl.outputs {
		if rw, ok := provider.Logger.(*redisWriter); ok {
			r = rw
			break
		}
	}
	...
	psc := redis.PubSubConn{Conn: r.conns.Get()}
	defer psc.Close()
	psc.Subscribe(r.Key)
	defer psc.Unsubscribe(r.Key)
	go func() {
		for {
	...
	for {
		...
		case redis.Message:
			...
			case r.Key:
				...
				if filteData(string(n.Data), include, exclude) != "" {
					if err := ws.WriteMessage(websocket.TextMessage,
						n.Data); err != nil {
						return
					}
...
func filteData(data, include, exclude string) string {
	includeArr := strings.Split(include, ",")
	excludeArr := strings.Split(exclude, ",")
	isKeyData := false
	for _, in := range includeArr {
		if in != "" && strings.Contains(data, in) {
			isKeyData = true
		} else {
			isKeyData = false
		}
	}
	for _, ex := range excludeArr {
		if ex != "" && strings.Contains(data, ex) {
			isKeyData = false
		}
	}
	if isKeyData {
...
// Tailing tails logs from redis adapter.
func Tailing(ws *websocket.Conn, include, exclude string) {
	beeLogger.Tailing(ws, include, exclude)
}
